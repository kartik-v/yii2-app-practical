...
第一个命令安装composer asset插件，它允许通过Composer管理bower和npm包依赖。 您只需要为所有运行此命令一次。 第二个命令将高级应用程序安装在名为 `practical` 的目录中。 如果需要，您可以选择不同的目录名称。 
...
3. 打开控制台终端，执行迁移命令 `/path/to/php-bin/php /path/to/practical/yii migrate`.

4. 设置Web服务器的文档根目录：

   - 对于前端 `/path/to/practical/` 并且使用URL `http://frontend.dev/`
   - 对于后端 `/path/to/practical/backend/web/` 并且使用URL `http://backend.dev/`
...
5. 准备项目:
   * 下载 [yii2-app-practical](https://github.com/yiisoft/yii2-app-practical/archive/master.zip)
   * 解压
   * 进入 `yii2-app-practical-master/vagrant/config` 文件夹
