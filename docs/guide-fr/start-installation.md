...
qui permet de gérer les dépendances de paquets bower et npm via Composer. Vous n'avez besoin d'exécuter cette commande qu'une fois pour toute. La deuxième commande installe l'application avancée dans un dossier nommé `practical`. Vous avez le droit de choisir un autre nom de dossier si vous le désirez.
...
3. Ouvrez un terminal, appliquez les migrations avec la commande `/path/to/php-bin/php /path/to/practical/yii migrate`.

4. Définissez la racine du document de votre serveur Web :

   - pour l'interface utilisateur (frontend)  `/path/to/practical/frontend/`, en utilisant l'URL `http://frontend.dev/`
   - pour l'interface d'administration (backend) `/path/to/practical/backend/web/`, en utilisant URL `http://backend.dev/`
...
5. Préparez le projet :
   * téléchargez le dépôt [yii2-app-practical](https://github.com/yiisoft/yii2-app-practical/archive/master.zip)
   * décompressez-le avec unzip
   * placez-vous dans le dossier `yii2-app-practical-master/vagrant/config`
